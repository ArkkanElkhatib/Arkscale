...
if uploaded_file is not None:
    if uploaded_file.size >= 100_000:
        st.warning('Due to _computational_ (financial) restraints, please only upscale images <= 0.1MB in size')
        uploaded_file = None
    else:
        ...
        if st.button('Upscale'):
            ...
            t.start()
            with st.spinner('Upscaling Image... Please wait...', show_time=True):
                time.sleep(1.5)
            ret = t.join()
